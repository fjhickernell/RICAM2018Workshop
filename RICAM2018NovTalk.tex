...
\begin{frame}{Series Spaces}

\vspace{-5ex}


\begin{align*}
    \cf &:= \left \{ f = \sum_{\vi \in \cn} \hf(\vi) u_{\vi} : \norm[\cf]{f} := \bignorm[r,\vlambda]{\hf} : = \norm[r]{\left(\frac{\bigabs{\hf(\vi)}}{\lambda_{\vi}} \right)_{\vi \in \cn}} \right \} \\
    \cg &: = \left \{ g = \sum_{\vi \in \cn} \hg(\vi) v_{\vi} : \norm[\cg]{g} := \bignorm[t]{\hg}\right \}, \qquad S(u_{\vi}) = v_{\vi}, \\
    \text{E.g., } & \text{ Function recovery: } S(u_{\vi}) = u_{\vi} = v_{\vi} \\
    & \text{ Integration: } S(u_{\vi}) = \int_{[0,1]^d} u_\vi (\vx) \, \dif \vx  = v_\vi\\
    \norm[\cg]{S(f) - A(f,\varepsilon)} & \le \sum_{\vi \in \cn} \bigabs{\hf(\vi)} \norm[\cg]{v_\vi - \Sapp(u_{\vi},n)}
\end{align*}
If $\gamma_{\vi_1} \ge \gamma_{\vi_2} \ge \cdots$, then $S_{\textup{app},n}(f) = \sum_{j=1}^n \hf(\vi_j) v_{\vi_j}$ is an optimal building block, and 
\begin{equation*}
    \norm[t]{S(f) - S_{\textup{app},n}(f)} = \Bignorm[r']{\left(\hf(\vi_j)\right)_{j=n+1}^{\infty}} \le \bignorm[r,\vgamma]{\hf} \Bignorm[qq'/(q-q')]{\left(\gamma_{\vi_j}\right)_{j=n+1}^{\infty}}
\end{equation*}
