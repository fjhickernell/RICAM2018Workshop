...
\centerline{
    \begin{tabular}{>{\vspace{-1.7ex}\raggedleft}p{0.2\textwidth}@{\qquad}p{0.3\textwidth}@{\qquad}p{0.3\textwidth}}
    & \alert{Previous talks} & \alert{My reponse} \tabularnewline
    \toprule
    \alert{Henryk, Klaus, Greg, Stefan, \ldots} & Many results for $f \in \smallscoop$  &
    May we obtain analogous results for $f \in \smallcone$? \tabularnewline[2ex]
    ...
    \alert{Paul} & Let's \alert{find} the Lipschitz matrix constant \ldots & Yes, lets learn from the function data \tabularnewline[2ex]
...
\begin{align*}
    ...
    u_{\vj} = \text{Cosine/Sine or Walsh}, \qquad S(u_{\vj}) = \int_{[0,1]^d} u_\vj (\vx) \, \dif \vx = \delta_{\vj,\vzero}\\
...
%\footfullcite{WuHam00,KuoEtal12a}
...
\end{cases}
\text{\alert{POSD weights\footfullcite{WuHam00,KuoEtal12a}}} \\[-2ex]
      & \alert{\lambda_{\vj_1} \ge \lambda_{\vj_2} \ge \cdots}, \qquad
...
\begin{frame}{ChanSun Test Function\footfullcite{VirLib17a}}
...
\begin{itemize}
    ...
    \item Much to be done to 
    \begin{itemize}
        \item demonstrate \alert{solvability}
        
        \item \alert{construct} algorithms
        
        \item derive upper bounds on \alert{computational cost}
        
        \item derive lower bounds on \alert{computational complexity}
        
        \item determine conditions on \alert{tractability}
        
        \item \alert{implement} algorithms in available software
    \end{itemize}
