...
\begin{itemize}
    ...
    \item Philosophy of our algorithms:  \alert{What we cannot observe is not much worse than what we can observe}
...
\begin{frame}{Solving General Linear Problems Using Series Coefficients\footfullcite{DinHic20a}}

\vspace{-7ex}

\begin{align*}
    \cf &:= \left \{ f = \sum_{\vj \in \cn} \hf(\vj) u_{\vj} : \norm[\cf]{f} := \bignorm[2,\vlambda]{\hf} : = \norm[2]{\left(\frac{\bigabs{\hf(\vj)}}{\lambda_{\vj}} \right)_{\vj \in \cn}} \right \} \qquad \begin{minipage}{3cm}\raggedright\alert{$\vlambda$ affects convergence rate \& tractabililty}\end{minipage}\\
    \cg &: = \biggl \{ g = \sum_{\vj \in \cn} \hg(\vj) v_{\vj} : \norm[\cg]{g} := \bignorm[2]{\hg}\biggr \}, \qquad S(u_{\vj}) = v_{\vj} \\
      \lambda_{\vj_1} & \ge \lambda_{\vj_2} \ge \cdots, \qquad
