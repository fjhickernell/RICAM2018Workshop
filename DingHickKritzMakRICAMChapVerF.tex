...
\end{equation*}
so $\tau \le \rho' \le \infty$.  Let $\Lambda :=  \bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK}}$ be the norm of a subset of the weights.  Then the following are true for $f = \sum_{\bsk \in \calK} \DHKMhf(\bsk) u_{\bsk}$:
\begin{equation}
\label{DHKM:SOL_ineq}
...
\begin{align}
    ...
    & = \biggnorm[\calG]{\sum_{\bsk \in \calK} \DHKMhf(\bsk) v_{\bsk}} = \DHKMnorm[\tau]{\bigl(  \DHKMhf(\bsk)  \bigr)_{\bsk \in \calK}}  = \left [\sum_{\bsk \in \calK}  \left\lvert\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right\rvert^{\tau} \lambda_{\bsk}^{\tau} \right]^{1/\tau} \\
    \nonumber
    & \le \biggnorm[\rho]{\biggl(  \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}}  \biggr)_{\bsk \in \calK}} \, \bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK}} = \DHKMnorm[\calF]{f} \, \Lambda \qquad \text{since }\frac 1\rho + \frac 1 {\rho'} = \frac 1 \tau.
\end{align}
...
\begin{equation} \label{DHKM:SOLNorm}
    \DHKMnorm[\calF \to \calG]{\DHKMSOL}  = \sup_{\DHKMnorm[\calF]{f} \le 1} \DHKMnorm[\calG]{\DHKMSOL(f)} = \bignorm[\rho']{\bslambda}.
\end{equation}
...
\begin{equation} \label{DHKM:APP_errorBd}
    \sup_{f \in \calB_R} \DHKMnorm[\calG]{\DHKMSOL(f) - \DHKMAPP(f,n)}  \le R \, \bignorm[\rho']{\bslambda^{(n+1)}}.
\end{equation}
...
\begin{multline} \label{DHKM:PilotConeAlg_comp}
        \DHKMCOMP(\calC_{\boldsymbol{\lambda},n_1,A},\varepsilon,R) \ge \min \left \{n \ge n_1 : \bignorm[\rho']{\bslambda^{(n+1)}} \,
...
\begin{equation*}
    \DHKMbigabs{\DHKMhf_1(\bsk)} = \begin{cases} \displaystyle \frac{R (1+1/A) \lambda_{\bsk}^{\rho'/\rho + 1}}{2\bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK_1}}^{\rho'/\rho}}, &  \bsk \in \calK_1, \\
    0, & \bsk \notin \calK_1,
    \end{cases}
...
\begin{gather}
\nonumber
    \DHKMbigabs{\DHKMhf_2(\bsk)} = \begin{cases} \displaystyle \frac{R (1-1/A) \lambda_{\bsk}^{\rho'/\rho + 1}}{2\bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \notin \calJ}}^{\rho'/\rho}}, &  \bsk \notin \calJ, \\
    0, & \bsk \in \calJ, 
    \end{cases}
    \qquad \DHKMnorm[\calF]{f_2} = \frac{R(1 - 1/A)}{2}, \\
    \label{DHKM:SOLf2bd}
    \DHKMnorm[\calG]{\DHKMSOL(f_2)} = \frac{R(1 - 1/A)}{2} \, \bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \notin \calJ}}.
\end{gather}
...
% \[
% n_{\DHKMCOMP}(\delta,d) :=\min \left \{n\ge 0: \bignorm[\rho']{\bigl(  \lambda_{d,\bsk_i}  \bigr)_{i = n+1}^{\infty}} \,
%     \le \delta \right \} \qquad \forall \delta > 0.
% \]
\[
n_{\DHKMCOMP}(\delta,d) :=\min \left \{n\ge 0: \bignorm[\rho']{\bslambda^{(n+1)}_d} \,
    \le \delta \right \} \qquad \forall \delta > 0,
\]
...
\begin{equation} \label{DHKM:NormLambdaFinite}
    \DHKMnorm[\calF \to \calG]{\DHKMSOL} = \bignorm[\rho']{\bigl(\Lambda_j \bigr)_{j \in \N_0}} < \infty.
\end{equation}

If $f$ belongs to the $\calC_{\boldsymbol{\lambda},\bsn,a,b}$ defined in \eqref{DHKM:TrackConeDef} and $n_0 = 0$, then 
\begin{align*}
    \DHKMnorm[\calF]{f} & = \DHKMnorm[\rho]{\Biggl(\,\DHKMnorm[\rho]{\biggl(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \calK_j}}\,\Biggr)_{j \in \N}} = \bignorm[\rho]{\bigl(\sigma_j(f) \bigr)_{j \in \N}} \\
...
\begin{equation*}
    \fapp = \sum_{\bsk \in \bar{\calK}} \DHKMhf(\bsk) u_{\bsk}.
\end{equation*}
...
\State Evaluate initial sample $\bigl\{\DHKMhf(\bsk)\bigr\}_{\bsk \in \bcalK}$
