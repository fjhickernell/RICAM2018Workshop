...
\algnewcommand\DHKMalgorithmicparam{\textbf{Parameters:}}
\algnewcommand\DHKMPARAM{\item[\DHKMalgorithmicparam]}
\algnewcommand\DHKMalgorithmicinput{\textbf{Input:}}
\algnewcommand\DHKMINPUT{\item[\DHKMalgorithmicinput]}
\algnewcommand\DHKMRETURN{\State \textbf{Return }}
...
\newcommand{\DHKMbigabs}[1]{\ensuremath{\bigl \lvert #1 \bigr \rvert}}
...
\newcommand{\DHKMbiggnorm}[2][{}]{\ensuremath{\biggl \lVert #2 \biggr \rVert}_{#1}}
\newcommand{\DHKMbcalK}{\bar{\calK}}
\newcommand{\DHKMfapp}{f_{\text{app}}}
...
\end{equation*}
so $\tau \le \rho' \le \infty$.  Let $\Lambda :=  \DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK}}$ be the norm of a subset of the weights.  Then the following are true for $f = \sum_{\bsk \in \calK} \DHKMhf(\bsk) u_{\bsk}$:
\begin{equation}
\label{DHKM:SOL_ineq}
    \DHKMnorm[\calG]{\DHKMSOL(f)} = \DHKMnorm[\tau]{\bigl(\DHKMhf(\bsk) \bigr)_{\bsk \in \calK}} \le \DHKMnorm[\calF]{f} \, \Lambda,
    \end{equation}
    \begin{multline}
    \label{DHKM:SOL_tight_ineq}
    \DHKMbigabs{\DHKMhf(\bsk)} = \begin{cases}
    \displaystyle 
    ...
    \end{cases}
   \\ 
    \implies  \ \ \DHKMnorm[\calF]{f} = R \ \mbox{and} \ \DHKMnorm[\calG]{\DHKMSOL(f)} = R \Lambda.
    \end{multline}
...
\begin{align}
    \label{DHKM:SOL_A}
    \DHKMnorm[\calG]{\DHKMSOL(f)}  
    & = \DHKMbiggnorm[\calG]{\sum_{\bsk \in \calK} \DHKMhf(\bsk) v_{\bsk}} = \DHKMnorm[\tau]{\bigl(  \DHKMhf(\bsk)  \bigr)_{\bsk \in \calK}}  = \left [\sum_{\bsk \in \calK}  \left\lvert\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right\rvert^{\tau} \lambda_{\bsk}^{\tau} \right]^{1/\tau} \\
    \nonumber
    & \le \DHKMbiggnorm[\rho]{\biggl(  \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}}  \biggr)_{\bsk \in \calK}} \, \DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK}} = \DHKMnorm[\calF]{f} \, \Lambda \qquad \text{since }\frac 1\rho + \frac 1 {\rho'} = \frac 1 \tau.
\end{align}
Substituting the formula for $\DHKMbigabs{\DHKMhf(\bsk)}$ in \eqref{DHKM:SOL_tight_ineq} into equation \eqref{DHKM:SOL_A} and applying the relationship between $\rho$, $\rho'$, and $\tau$ yields
\begin{equation*}
       \DHKMnorm[\calG]{\DHKMSOL(f)}  
    =  \frac{R \DHKMbignorm[\tau]{\bigl(  \lambda_{\bsk}^{\rho'/\rho + 1}  \bigr)_{\bsk \in \calK}}} {\Lambda^{\rho'/\rho}} 
    = \frac{R \DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK}}^{\rho'/\rho + 1}}
    {\Lambda^{\rho'/\rho}} = R \Lambda.
\end{equation*}
Moreover,
\begin{equation*}
    \DHKMnorm[\calF]{f}  
    = \DHKMnorm[\rho]{\left( \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right)_{\bsk \in \calK}}
    = \frac{R \DHKMbignorm[\rho]{\bigl(  \lambda_{\bsk}^{\rho'/\rho}  \bigr)_{\bsk \in \calK}}}{\Lambda^{\rho'/\rho}} 
    = \frac{R \DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK}}^{\rho'/\rho}}
    {\Lambda^{\rho'/\rho}} = R.
\end{equation*}
...
\begin{equation} \label{DHKM:SOLNorm}
    \DHKMnorm[\calF \to \calG]{\DHKMSOL}  = \sup_{\DHKMnorm[\calF]{f} \le 1} \DHKMnorm[\calG]{\DHKMSOL(f)} = \DHKMbignorm[\rho']{\bslambda}.
\end{equation}
...
\begin{equation} \label{DHKM:SOLNormFinite}
    \DHKMbignorm[\rho']{\bslambda} < \infty.
\end{equation}
...
\begin{equation} \label{DHKM:APP_def}
    \DHKMAPP : \calF \times \N_0 \to \calG, \quad  \DHKMAPP(f,0) = 0, \ \ \DHKMAPP(f,n) := \sum_{i=1}^n \DHKMhf(\bsk_i) v_{\bsk_i} \ \forall n \in \N.
\end{equation}
By the argument leading to \eqref{DHKM:SOL_A} it follows that 
\begin{equation} \label{DHKM:APP_Err_Coef}
    \DHKMnorm[\calG]{\DHKMSOL(f) - \DHKMAPP(f,n)} = \DHKMnorm[\tau]{\bigl(\DHKMhf(\bsk_i)\bigr)_{i=n+1}^\infty}.
\end{equation}
An upper bound on the approximation error follows from Lemma \ref{DHKM:Key_Lem}:
\begin{gather} \label{DHKM:Refined_APP_err}
    \DHKMnorm[\calG]{\DHKMSOL(f) - \DHKMAPP(f,n) } \le \DHKMnorm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=n+1}^\infty}
    \DHKMbignorm[\rho']{\bslambda^{(n+1)}}, \\
...
\begin{theorem} \label{DHKM:APP_optimality_thm} Let $\calB_{R} : = \{ f \in \calF : \DHKMnorm[\calF]{f} \le R \}$ denote the ball of radius $R$ in the space of input functions.  The error of the approximation defined in \eqref{DHKM:APP_def} is bounded tightly above as 
\begin{equation} \label{DHKM:APP_errorBd}
    \sup_{f \in \calB_R} \DHKMnorm[\calG]{\DHKMSOL(f) - \DHKMAPP(f,n)}  \le R \, \DHKMbignorm[\rho']{\bslambda^{(n+1)}}.
\end{equation}
...
\caption{Non-Adaptive $\DHKMALG$ for a Ball of Input Functions \label{DHKM:BallAlg}}
	\begin{algorithmic}
	\DHKMPARAM the Banach spaces $\calF$ and $\calG$, including the weights $\bslambda$; the ball radius, $R$; $\DHKMAPP$ satisfying \eqref{DHKM:APP_errorBd}
	\DHKMINPUT a black-box function, $f$; an absolute error tolerance, $\varepsilon>0$

    \Ensure Error criterion \eqref{DHKM:err_crit} for $\calC = \calB_{R}$

    \State Choose $n^* =  \min \left \{n \in \N_0 : \DHKMbignorm[\rho']{\bslambda^{(n+1)}} \le \varepsilon /R \right \}$

    \DHKMRETURN $\DHKMALG(f,\varepsilon) = \DHKMAPP(f,n^*)$
\end{algorithmic}
\end{algorithm}
...
\[
\DHKMCOST(\DHKMALG, \calB_R, \varepsilon,R') = \min \left \{n \in \N_0 : \DHKMbignorm[\rho']{\bslambda^{(n+1)}} \le \varepsilon /R \right \}, \qquad R’ \in (0, R].
\]
This algorithm is essentially optimal for the set of input functions $\calB_R$, namely,
\begin{multline*}
\DHKMCOST(\DHKMALG, \calB_R, \varepsilon,R') \le \DHKMCOMP(\calB_R,\omega \varepsilon ,R') \\ \forall \, \varepsilon \in (0, \varepsilon_{\max}], \ R’ \in  [ R_{\min} , R], \ R \in [R_{\min}, \infty), 
\end{multline*}
...
Accordingly, we now define the cone of functions whose norm can be bounded well in terms of a pilot sample, $\{\DHKMhf(\bsk_1), \ldots, \DHKMhf(\bsk_{n_1})\}$. It is given by
\begin{equation} \label{DHKM:pilot_cone}
    \calC_{\boldsymbol{\lambda},n_1,A} = \left \{ f \in \calF : \DHKMnorm[\calF]{f} \le A \DHKMnorm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \right\}.
\end{equation}
...
\begin{align*}
     \MoveEqLeft{\DHKMnorm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=n+1}^\infty}} \\
     & =  \left[ \DHKMnorm[\calF]{f}^\rho -  \DHKMnorm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=1}^n}^\rho
    \right]^{1/\rho} \qquad \forall f \in \calF \\
    &  \le  \left[ A^\rho \DHKMnorm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}}^\rho -  \DHKMnorm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=1}^n}^\rho
...
\begin{equation}
\DHKMnorm[\calG]{\DHKMSOL(f) - \DHKMAPP(f,n)}  \le \DHKMERRN  \qquad \forall f \in \calC_{\boldsymbol{\lambda},n_1,A},
\end{equation}
where 
\begin{multline} \label{ErrNDef}
\DHKMERRN \\
    : =  
    \left[ A^\rho \DHKMnorm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}}^\rho -  \DHKMnorm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=1}^n}^\rho \right]^{1/\rho} 
    \, \DHKMbignorm[\rho']{\bslambda^{(n+1)}} , 
    \\ \forall n \ge n_1.
\end{multline}
\end{subequations}
This error bound decays as $\DHKMbignorm[\rho]{\bigl( f(\bsk_i)/\lambda_{\bsk_i} \bigr)_{i=1}^n}$ increases and as the tail norm of the $\lambda_{\bsk_i}$ decreases.  This data-driven error bound underlies  Algorithm \ref{DHKM:PilotConeAlg}, which is successful for the cone of input functions, $\calC_{\boldsymbol{\lambda},n_1,A}$, defined in \eqref{DHKM:pilot_cone}:

\begin{algorithm}[H]
	\caption{$\DHKMALG$ Based on a Pilot Sample\label{DHKM:PilotConeAlg}} 
	\begin{algorithmic}
	\DHKMPARAM the Banach spaces $\calF$ and $\calG$, including the weights $\bslambda$; an initial sample size, $n_1 \in \N$; an inflation factor, $A > 1$; $\DHKMAPP$ satisfying \eqref{DHKM:Refined_APP_err}
...
\begin{multline} \label{DHKM:PilotConeAlg_cost}
    ...
    = \min \left \{n \ge n_1 : \DHKMbignorm[\rho']{\bslambda^{(n+1)}} \,
...
\begin{multline} \label{DHKM:PilotConeAlg_comp}
        \DHKMCOMP(\calC_{\boldsymbol{\lambda},n_1,A},\varepsilon,R) \ge \min \left \{n \ge n_1 : \DHKMbignorm[\rho']{\bslambda^{(n+1)}} \,
...
\begin{align*}
    ...
    & = \max_{f \in \calC \cap \calB_{R}} \min \left \{n \ge n_1 : \DHKMERRN \le \varepsilon \right \} \\
     ...
     (A^\rho -1)^{1/\rho} \DHKMnorm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \, 
     \DHKMbignorm[\rho']{\bslambda^{(n+1)}}
    \le \varepsilon \right \} \\   
     & \le \min \left \{n \ge n_1 : 
     (A^\rho -1)^{1/\rho} R \,\DHKMbignorm[\rho']{\bslambda^{(n+1)}}
    \le \varepsilon \right \},  
\end{align*}
since $\DHKMbignorm[\rho]{\bigl( \DHKMhf(\bsk_i)/\lambda_{\bsk_i} \bigr)_{i=1}^{n_1}} \le \DHKMbignorm[\rho]{\bigl( \DHKMhf(\bsk_i)/\lambda_{\bsk_i} \bigr)_{i=1}^{n}} \le  \DHKMnorm[\calF]{f} \le R$ for all $f \in \calB_R$, $n \ge n_1$.  Moreover, this inequality is tight for some $f \in \calC_{\boldsymbol{\lambda},n_1,A} \cap \calB_{R}$, namely, those certain $f$ for which $\DHKMhf(\bsk_i) = 0$ for $i > n_1$.  This completes the proof of \eqref{DHKM:PilotConeAlg_cost}.

To prove the lower complexity bound, choose $\varepsilon_{\max}$ and $R_{\min}$ such that 
\[
\DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk_i}  \bigr)_{i = n_1+1}^{\infty}} \,
...
\begin{equation*}
    \DHKMbigabs{\DHKMhf_1(\bsk)} = \begin{cases} \displaystyle \frac{R (1+1/A) \lambda_{\bsk}^{\rho'/\rho + 1}}{2\DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK_1}}^{\rho'/\rho}}, &  \bsk \in \calK_1, \\
    0, & \bsk \notin \calK_1,
    \end{cases}
   \qquad \DHKMnorm[\calF]{f_1} = \frac{R(1 + 1/A)}{2}.
\end{equation*}
Suppose that $\DHKMALG'(f_1,\varepsilon)$ samples the series coefficients $\DHKMhf_1(\bsk)$ for $\bsk \in \calJ$, and let $n$ denote the cardinality of $\calJ$.  

Now, construct the input function $f_2$, having zero coefficients for $\bsk \in \calJ$ and also as in Lemma \ref{DHKM:Key_Lem}:
\begin{gather}
\nonumber
    \DHKMbigabs{\DHKMhf_2(\bsk)} = \begin{cases} \displaystyle \frac{R (1-1/A) \lambda_{\bsk}^{\rho'/\rho + 1}}{2\DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \notin \calJ}}^{\rho'/\rho}}, &  \bsk \notin \calJ, \\
    0, & \bsk \in \calJ, 
    \end{cases}
    \qquad \DHKMnorm[\calF]{f_2} = \frac{R(1 - 1/A)}{2}, \\
    \label{DHKM:SOLf2bd}
    \DHKMnorm[\calG]{\DHKMSOL(f_2)} = \frac{R(1 - 1/A)}{2} \, \DHKMbignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \notin \calJ}}.
\end{gather}

Let $f_{\pm} = f_1 \pm f_2$.  By the definitions above, it follows that
\begin{align}
\nonumber
    \DHKMnorm[\calF]{f_{\pm}} &= \DHKMnorm[\calF]{ f_1 \pm f_2 } \le \DHKMnorm[\calF]{ f_1} + \DHKMnorm[\calF]{ f_2 } =  R, \\
    \nonumber
    \DHKMnorm[\rho]{\left( \frac{\DHKMhf_\pm(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} 
    & = \DHKMnorm[\rho]{\left( \frac{\DHKMhf_1(\bsk_i) \pm \DHKMhf_2(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \\
    \nonumber
    & \ge \DHKMnorm[\rho]{\left( \frac{\DHKMhf_1(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} - \DHKMnorm[\rho]{\left( \frac{\DHKMhf_2(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \\
    \nonumber
    & \ge \DHKMnorm[\calF]{ f_1} - \DHKMnorm[\calF]{ f_2 } =  \frac{R}{A} \ge \frac{\DHKMnorm[\calF]{f_{\pm}}}{A}.
\end{align}
...
\[ 
\lambda_{\bsk_{n+1}} \le \frac{\DHKMnorm[p]{\bslambda}}{n^{1/p}} \ \ \text{and} \ \ 
 \sup_{n \ge n_{\min}} \lambda_{\bsk_{n}} n^{1/p} \ge \frac{\DHKMbignorm[p\eta]{\bslambda^{(n_{\min})}}}{\zeta(\eta)^{1/(p\eta)}} \quad \forall p>0, \ \eta > 1, \ n_{\min} \in \bbN.
\]
If $\DHKMnorm[p]{\bslambda} = \infty$ or $\sup_{n \ge n_{\min}} \lambda_{\bsk_{n}} n^{1/p} = \infty$, we consider these inequalities to be trivially true.
\end{lemma}
\begin{proof}
...
\begin{equation*}
    ...
    + \lambda_{\bsk_{n}}^p  \le \sum_{\bsk \in \bbK} \lambda_{\bsk}^p = \DHKMnorm[p]{\bslambda}^p \qquad \forall p > 0.
\end{equation*}
...
\begin{equation*}
    \DHKMbignorm[p\eta]{\bslambda^{(n_{\min})}}^{p\eta} 
...
If the elements of $\bslambda$ decay fast enough so that $\DHKMnorm[p]{\bslambda} < \infty$ and $\rho'=\infty$, then the information costs of Algorithms \ref{DHKM:BallAlg} and \ref{DHKM:PilotConeAlg} are bounded above by
\begin{align*}
    \text{Algorithm \ref{DHKM:BallAlg}:} \quad& \DHKMCOST(\DHKMALG,\calB_R,\varepsilon, R') 
    \le \left \lceil  \left(\frac{R \DHKMnorm[p]{\bslambda}}{\varepsilon}\right)^p     \right \rceil, \qquad \forall \varepsilon, \ 0 < R' < R, \\
    \text{Algorithm \ref{DHKM:PilotConeAlg}:} \quad& \DHKMCOST(\DHKMALG,\calC_{\bslambda,n_1,A},\varepsilon, R) 
    \\ & \qquad \qquad \le \max \left( n_1, 
    \left \lceil  \left(\frac{[(A^\rho -1)^{1/\rho}R] \DHKMnorm[p]{\bslambda}}{\varepsilon}\right)^p \right \rceil \right).
    \end{align*}
...
\begin{align*}
    ...
    \end{align*}
then $\DHKMnorm[p\eta]{\bslambda} < \infty$ for all $\eta > 1$.  Since Algorithms \ref{DHKM:BallAlg} and \ref{DHKM:PilotConeAlg} are essentially optimal, the above inequalities provide upper and lower asymptotic bounds on  $\DHKMCOMP(\calB_R,\varepsilon,R')$ and $\DHKMCOMP(\calC_{\bslambda,n_1,A},\varepsilon,R)$.
\end{theorem}
\begin{proof}
We give the proof for Algorithm \ref{DHKM:PilotConeAlg}, which is similar to the proof for Algorithm \ref{DHKM:BallAlg}.  Recall that for $\rho'=\infty$, $\DHKMbignorm[\rho']{\bslambda^{(n+1)}} = \lambda_{\bsk_{n+1}}$. From \eqref{DHKM:PilotConeAlg_cost} it follows that the information cost for  Algorithm \ref{DHKM:PilotConeAlg} is
\begin{align*} 
    \DHKMCOST(\DHKMALG,\calC_{\boldsymbol{\lambda},n_1,A},\varepsilon,R)
    & = \min \left \{n \ge n_1 : \lambda_{\bsk_{n+1}}
    \le \frac{\varepsilon}{[(A^\rho -1)^{1/\rho}R]} \right \} \\
  & \le  \min \left \{n \ge n_1 : \DHKMnorm[p]{\bslambda}n^{-1/p}
    \le \frac{\varepsilon}{[(A^\rho -1)^{1/\rho}R]} \right \} \\
    & \qquad \qquad \text{by Lemma \ref{DHKM:lambdineq} since } \lambda_{\bsk_{n+1}} \le \DHKMnorm[p]{\bslambda}n^{-1/p}\\
  & = \left \lceil  \left(\frac{[(A^\rho -1)^{1/\rho}R] \DHKMnorm[p]{\bslambda}}{\varepsilon}\right)^p \right \rceil.
\end{align*}
...
Now suppose that $\DHKMCOST(\DHKMALG,\calC_{\bslambda,n_1,A},\varepsilon, R) 
    ...
    \begin{align*}
       \DHKMbignorm[p\eta]{\bslambda^{(n_{\min})}}  
    ...
    \end{align*}
    Since $\DHKMbignorm[p\eta]{\bslambda^{(n_{\min})}}$ is finite, it follows that $\DHKMbignorm[p\eta]{\bslambda}$ is finite.
\end{proof} \
...
we have the following lower and upper bounds on $\DHKMnorm[p]{\bslambda}$.  It holds that
\begin{gather*}
\DHKMnorm[p]{\bss} \DHKMnorm[p]{\bsw} \le \DHKMnorm[p]{\bslambda}\le \exp\left(\frac 1p \DHKMnorm[p]{\bss}^p \DHKMnorm[p]{\bsw}^p  \right) \qquad \text{for general } \bss, \\
[\zeta(pr)]^{1/p} \DHKMnorm[p]{\bsw} \le \DHKMnorm[p]{\bslambda} \le \exp\left(\frac {\zeta(pr)}p \DHKMnorm[p]{\bsw}^p  \right) \qquad \text{for } \bss = (1, 2^{-r}, \ldots ), \ pr > 1.
\end{gather*}
\end{lemma}
...
\begin{align*}
    \MoveEqLeft{\DHKMCOMP(\calB_{R},\varepsilon,R)} 
  \le \left \lceil \left( \frac{R}{\varepsilon} \right)^{p}  \exp\left(\zeta(p r) \DHKMnorm[p]{\bsw}^p  \right) \right \rceil .
\end{align*}
If $p^\dagger$ is the infimum of the $p$ for which $\DHKMnorm[p]{\bsw}$ is finite, and $p^\dagger$ is finite, then we obtain strong polynomial tractability and an exponent of strong tractability that is $\widetilde{p}_* = \max(1/r,p^\dagger)$. On the other hand, if the coordinate weights are all unity,  $w_1 = w_2 = \cdots = 1$, then there are $2^d$ different $\lambda_{\bsk}$ with a value of $1$, and so $\DHKMCOMP(\calB_{R},\varepsilon,R) \ge 2^d$, and the problem is not tractable.\\
...
% \[
% n_{\DHKMCOMP}(\delta,d) :=\min \left \{n\ge 0: \DHKMbignorm[\rho']{\bigl(  \lambda_{d,\bsk_i}  \bigr)_{i = n+1}^{\infty}} \,
%     \le \delta \right \} \qquad \forall \delta > 0.
% \]
\[
n_{\DHKMCOMP}(\delta,d) :=\min \left \{n\ge 0: \DHKMbignorm[\rho']{\bslambda^{(n+1)}_d} \,
    \le \delta \right \} \qquad \forall \delta > 0,
\]
...
\[
n_{\DHKMCOMP}(\varepsilon/R,d)=\min \left \{n\ge 0\colon \lambda_{d,\bsk_{n+1}} \,
...
Using the same notation as above, the following statements hold for the case $\rho'=\infty$.
 ...
 \begin{equation}\label{DHKM:eq:condspt}
    \sup_{d\in\NN} \, \DHKMbignorm[p]{\bslambda_d^{(i_0)}} < \infty.
 \end{equation}
 ...
 \[
    \sup_{d\in\NN} d^{-p_1}\, \DHKMbignorm[p_3]{\bslambda_d^{(\lceil K d^{p_2} \rceil)}} < \infty.
 \]
 \item[3.] 
...
$\DHKMbignorm[p]{\bslambda_d^{(i_0)}}^p<\infty$ holds if and only if  $\DHKMbignorm[p]{\bslambda_d}^p<\infty$.
...
\[
\DHKMnorm[p]{\bss} \DHKMnorm[p]{\bsw_d} \le \DHKMnorm[p]{\bslambda_d}\le \exp\left(\frac 1p \DHKMnorm[p]{\bss}^p \DHKMnorm[p]{\bsw_d}^p  \right),
\]
...
%\label{DHKM:sptexample}
    \DHKMnorm[p]{\bss} < \infty \text{ and } 
    \DHKMnorm[p]{\bsw_\infty} < \infty.
\end{equation*}
...
\begin{equation} \label{DHKM:LambdaDef}
    \DHKMnorm[\tau]{\bigl(\DHKMhf(\bsk) \bigr)_{\bsk \in \calK_j}} \le \sigma_j(f) \Lambda_j, \qquad \text{where } \Lambda_j : = \DHKMnorm[\rho']{\bigl(\lambda_{\bsk} \bigr)_{\bsk \in \calK_j}}.
\end{equation}
...
\begin{equation} \label{DHKM:NormLambdaFinite}
    \DHKMnorm[\calF \to \calG]{\DHKMSOL} = \DHKMbignorm[\rho']{\bigl(\Lambda_j \bigr)_{j \in \N_0}} < \infty.
\end{equation}

If $f$ belongs to the $\calC_{\boldsymbol{\lambda},\bsn,a,b}$ defined in \eqref{DHKM:TrackConeDef} and $n_0 = 0$, then 
\begin{align*}
    \DHKMnorm[\calF]{f} & = \DHKMnorm[\rho]{\Biggl(\,\DHKMnorm[\rho]{\biggl(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \calK_j}}\,\Biggr)_{j \in \N}} = \DHKMbignorm[\rho]{\bigl(\sigma_j(f) \bigr)_{j \in \N}} \\
    & \le \DHKMbignorm[\rho]{\bigl(\sigma_1(f), ab \sigma_1(f), ab^2 \sigma_1(f), \ldots \bigr)} \\
    & = \left(1 + \frac{a^\rho b^\rho}{1 - b^\rho} \right)^{1/\rho}  \DHKMnorm[\rho]{\biggl(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \calK_1}}.
\end{align*}
...
If the elements of $\bslambda$ decay fast enough so that $\DHKMnorm[p]{\bslambda} < \infty$ and $\rho'=\infty$, then the information cost of Algorithm \ref{DHKM:TrackConeAlg} is bounded above by
\begin{multline*}
\DHKMCOST(\DHKMALG,\calC_{\bslambda,\bsn,a,b},\varepsilon, R) \\
    \le 
    \min \left \{n_0 2^j , \ j \in \N_0 : 
    n_0 2^j \ge \left(\frac{ R a^{2} \DHKMnorm[p]{\bslambda} }{\varepsilon (1 - b^{\rho}/2^{\rho/p})^{1/\rho}} \right)^p
    \right\}.
    \end{multline*}
For the function spaces defined in the illustrative example in Section \ref{DHKM:secexamp}, for all $p > 1/r$:
\begin{multline*}
 \DHKMCOST(\DHKMALG,\calC_{\bslambda,\bsn,a,b},\varepsilon, R) \\
   \le \min \left \{n_0 2^j , \ j \in \N_0 : 
    n_0 2^j \ge \left(\frac{ R a^{2}\exp\left( \zeta(pr) \DHKMnorm[p]{\bsw}^p  \right)  }{\varepsilon (1 - b^{\rho}/2^{\rho/p})^{1/\rho}} \right)^p \right \}.
    \end{multline*}
...
\begin{equation} \label{DHKM:barKdef}
    \DHKMbcalK= \{ (0, \ldots, 0, k, 0, \ldots, 0): k = 0, \ldots, k_{\max}\},
\end{equation}
...
\begin{equation*}
    \DHKMfapp = \sum_{\bsk \in \bar{\calK}} \DHKMhf(\bsk) u_{\bsk}.
\end{equation*}
...
\text{where } (\bar{\bsw}, \bar{\bss} ) = \min \left\{ \argmin_{(\bsw,\bss) \in \calW \times \calS} \left\|\left( \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}(\bsw,\bss,\bsGamma)} \right)_{\bsk \in \bar{\calK}}\right\|_{\rho} \right \}.
\label{DHKM:eq:inf}
\end{multline}
...
The optimization in \eqref{DHKM:eq:inf} is nontrivial to solve numerically. In practice, we iteratively optimize over $\bsw$ and then $\bss$ until convergence is reached.  At each step of the iteration $\|(\hat{f}(\bsk)/\lambda_{\bsk})_{\bsk \in \DHKMbcalK}\|_{\rho}$ decreases.
...
\begin{algorithm}
	...
	\begin{algorithmic}
	\DHKMPARAM the bases $\{u_{\bsk}\}_{\bsk \in \N_0^d}$ and $\{v_{\bsk}\}_{\bsk \in \N_0^d}$; candidate sets $\calW$ and $\calS$ for product and smoothness weights; maximum smoothness degree, $k_{\max}$; order weights, $\boldsymbol{\Gamma}$; an inflation factor, $A > 1$; $\DHKMAPP$ satisfying \eqref{DHKM:Refined_APP_err} 
		\DHKMINPUT a black-box function, $f$; an absolute error tolerance,
		$\varepsilon>0$

\Ensure Error criterion \eqref{DHKM:err_crit} for the cone defined in \eqref{DHKM:pilot_cone2}
\State  Define the initial set of wavenumbers $\DHKMbcalK$ defined in \eqref{DHKM:barKdef}
\State Evaluate initial sample $\bigl\{\DHKMhf(\bsk)\bigr\}_{\bsk \in \DHKMbcalK}$
...
\DHKMRETURN $\DHKMALG(f,\varepsilon)$
\end{algorithmic}
\end{algorithm}
