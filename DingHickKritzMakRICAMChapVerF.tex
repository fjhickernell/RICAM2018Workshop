...
and
\begin{gather*}
    \calG := \left \{g = \sum_{\bsk \in \mathbb{K}} \DHKMhg(\bsk) v_{\bsk} : \norm[\calG]{g} : = \norm[\tau]{\bigl(  \DHKMhg(\bsk)  \bigr)_{\bsk \in \mathbb{K}}} < \infty \right\}, \quad\mbox{with}\quad 1 \le \tau \le \rho.
\end{gather*}
...
\begin{equation*}
      \norm[\calG]{\sum_{\bsk \in \N_0^d} \DHKMhg(\bsk) u_{\bsk}} : = \norm[2]{\bigl(\DHKMhg(\bsk) \bigr)_{\bsk \in \N_0^d}}.
\end{equation*}
...
\tLambda_j := \norm[\rho']{\bigl(\lambda_\bsk \bigr)_{\bsk \in \DHKMtcalK_j}} \le \Lambda_j, \\
\nonumber
\sigma_j(f_2) & = \norm[\rho]{\biggl(\frac{\DHKMhf_{2j}(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \DHKMtcalK_j}} 
