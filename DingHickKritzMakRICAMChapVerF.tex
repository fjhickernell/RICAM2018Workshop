...
$$
  \norm[\calF]{f}\le h (\DHKMhf(\bsk_1),\ldots,\DHKMhf (\bsk_{n_1}))
$$
...
\begin{gather*}
    \calF := \left \{f = \sum_{\bsk \in \mathbb{K}} \DHKMhf(\bsk) u_{\bsk} : \norm[\calF]{f} : = \norm[\rho]{\left( \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right)_{\bsk \in \mathbb{K}}} < \infty \right\}, \quad\mbox{with}\quad 1 \le \rho \le \infty, 
\end{gather*}
and
\begin{gather*}
...
\begin{equation}
\label{DHKM:SOL_ineq}
    \norm[\calG]{\DHKMSOL(f)} = \norm[\tau]{\bigl(\DHKMhf(\bsk) \bigr)_{\bsk \in \calK}} \le \norm[\calF]{f} \, \Lambda,
    \end{equation}
    \begin{multline}
    \label{DHKM:SOL_tight_ineq}
    \bigabs{\DHKMhf(\bsk)} = \begin{cases}
    \displaystyle 
...
\begin{align}
    \label{DHKM:SOL_A}
    \norm[\calG]{\DHKMSOL(f)}  
    & = \biggnorm[\calG]{\sum_{\bsk \in \calK} \DHKMhf(\bsk) v_{\bsk}} = \norm[\tau]{\bigl(  \DHKMhf(\bsk)  \bigr)_{\bsk \in \calK}}  = \left [\sum_{\bsk \in \calK}  \left\lvert\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right\rvert^{\tau} \lambda_{\bsk}^{\tau} \right]^{1/\tau} \\
    \nonumber
    & \le \biggnorm[\rho]{\biggl(  \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}}  \biggr)_{\bsk \in \calK}} \, \bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK}} = \norm[\calF]{f} \, \Lambda \qquad \text{since }\frac 1\rho + \frac 1 {\rho'} = \frac 1 \tau.
\end{align}
Substituting the formula for $\bigabs{\DHKMhf(\bsk)}$ in \eqref{DHKM:SOL_tight_ineq} into equation \eqref{DHKM:SOL_A} and applying the relationship between $\rho$, $\rho'$, and $\tau$ yields
\begin{equation*}
       \norm[\calG]{\DHKMSOL(f)}  
...
\begin{equation*}
    \norm[\calF]{f}  
    = \norm[\rho]{\left( \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right)_{\bsk \in \calK}}
...
\begin{equation}
\label{DHKM:IllExWts}
    \norm[\calF]{f} := \norm[2]{\left(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}}\right)_{\bsk \in \N_0^d}}, \qquad \text{where } \lambda_\bsk := \prod_{\substack{\ell =1\\ k_\ell > 0}}^d \frac{w_\ell}{k^r_\ell}, \quad r > 0.
\end{equation}
...
\begin{equation*}
    \norm[\calG]{\DHKMSOL(f) - \DHKMAPP(f,n)} = \norm[2]{\bigl(\DHKMhf(\bsk_i) \bigr)_{i=n+1}^{\infty}}.
\end{equation*}
...
\begin{gather}
\nonumber
    \DHKMhf(\bsk'_1) = \cdots = \DHKMhf(\bsk'_n) = 0, \qquad \norm[\calF]{f} = R, \\ 
...
\begin{equation} \label{DHKM:pilot_cone}
    \calC_{\boldsymbol{\lambda},n_1,A} = \left \{ f \in \calF : \norm[\calF]{f} \le A \norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \right\}.
\end{equation}
...
\begin{align*}
     \MoveEqLeft{\norm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=n+1}^\infty}} \\
     & =  \left[ \norm[\calF]{f}^\rho -  \norm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=1}^n}^\rho
    \right]^{1/\rho} \qquad \forall f \in \calF \\
    &  \le  \left[ A^\rho \norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}}^\rho -  \norm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=1}^n}^\rho
...
\DHKMERRN \\
    : =  
    \left[ A^\rho \norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}}^\rho -  \norm[\rho]{\left(\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}\right)_{i=1}^n}^\rho \right]^{1/\rho} 
    \, \bignorm[\rho']{\bslambda^{(n+1)}} , 
    \\ \forall n \ge n_1.
\end{multline}
\end{subequations}
...
\begin{align*}
    ...
    & = \max_{f \in \calC \cap \calB_{R}} \min \left \{n \ge n_1 : \DHKMERRN \le \varepsilon \right \} \\
     ...
     (A^\rho -1)^{1/\rho} \norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \, 
     \bignorm[\rho']{\bslambda^{(n+1)}}
    \le \varepsilon \right \} \\   
    ...
    \le \varepsilon \right \},  
\end{align*}
since $\bignorm[\rho]{\bigl( \DHKMhf(\bsk_i)/\lambda_{\bsk_i} \bigr)_{i=1}^{n_1}} \le \bignorm[\rho]{\bigl( \DHKMhf(\bsk_i)/\lambda_{\bsk_i} \bigr)_{i=1}^{n}} \le  \norm[\calF]{f} \le R$ for all $f \in \calB_R$, $n \ge n_1$.  Moreover, this inequality is tight for some $f \in \calC_{\boldsymbol{\lambda},n_1,A} \cap \calB_{R}$, namely, those certain $f$ for which $\DHKMhf(\bsk_i) = 0$ for $i > n_1$.  This completes the proof of \eqref{DHKM:PilotConeAlg_cost}.
...
\begin{equation*}
    \bigabs{\DHKMhf_1(\bsk)} = \begin{cases} \displaystyle \frac{R (1+1/A) \lambda_{\bsk}^{\rho'/\rho + 1}}{2\bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \in \calK_1}}^{\rho'/\rho}}, &  \bsk \in \calK_1, \\
    0, & \bsk \notin \calK_1,
    \end{cases}
...
\begin{gather}
\nonumber
    \bigabs{\DHKMhf_2(\bsk)} = \begin{cases} \displaystyle \frac{R (1-1/A) \lambda_{\bsk}^{\rho'/\rho + 1}}{2\bignorm[\rho']{\bigl(  \lambda_{\bsk}  \bigr)_{\bsk \notin \calJ}}^{\rho'/\rho}}, &  \bsk \notin \calJ, \\
    0, & \bsk \in \calJ, 
    \end{cases}
...
\nonumber
    ...
    \nonumber
    \norm[\rho]{\left( \frac{\DHKMhf_\pm(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} 
    & = \norm[\rho]{\left( \frac{\DHKMhf_1(\bsk_i) \pm \DHKMhf_2(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \\
    \nonumber
    & \ge \norm[\rho]{\left( \frac{\DHKMhf_1(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} - \norm[\rho]{\left( \frac{\DHKMhf_2(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i=1}^{n_1}} \\
    \nonumber
...
\begin{equation} \label{DHKM:PilotConeNecessary}
    \norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i =1}^{n}} \le A
    \norm[\rho]{\left( \frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right)_{\bsk \in \calK_1}} \qquad \forall n \in \NN.
\end{equation}
...
\begin{multline} \label{DHKM:altconedef}
    \calC'_{\boldsymbol{\lambda}, \boldsymbol{\omega}, A'}:=\{ f \in \calF : \norm[\calF]{f} \le A'\norm[\calF']{f}\}, \\ \text{where } \norm[\calF']{f} :=\norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)\omega_{i}}{\lambda_{\bsk_i}} \right)_{i =  1}^n} \text{ and } \omega_1 \ge \omega_2 \ge \cdots.
\end{multline}
...
\begin{equation*}
    \norm[\calF]{f} \le A \norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}} \right)_{i = 1} ^{ n_1}}
     \le A \omega_{n_1}^{-1} \norm[\rho]{\left( \frac{\DHKMhf(\bsk_i)\omega_{\bsk_i}}{\lambda_{\bsk_i}} \right)_{i =1} ^{n_1}}
     \le A \omega_{n_1}^{-1} \norm[\calF']{f}.
\end{equation*}
...
\begin{align*}
    \norm[\calF']{f}^\rho- \norm[\rho]{\biggl ( \frac{\DHKMhf(\bsk_i) \omega_i }{\lambda_{\bsk_i}} \biggr) _{i=1}^{n_1}}^\rho 
	& = \norm[\rho]{\biggl ( \frac{\DHKMhf(\bsk_i) \omega_i }{\lambda_{\bsk_i}} \biggr) _{i=n_1 + 1}^{\infty}}^\rho 
     \le \omega_{n_1+1}^\rho \norm[\rho]{\biggl ( \frac{\DHKMhf(\bsk_i) }{\lambda_{\bsk_i}} \biggr) _{i=n_1 + 1}^{\infty}}^\rho \\
     & = \omega_{n_1+1}^\rho \left[ \norm[\calF]{f}^\rho - \norm[\rho]{\biggl ( \frac{\DHKMhf(\bsk_i) }{\lambda_{\bsk_i}} \biggr) _{i=1}^{n_1}}^\rho \right ]\\	
     & \le \omega_{n_1+1}^\rho \left[ A'{}^\rho \norm[\calF']{f}^\rho - \omega_1^{-\rho} \norm[\rho]{\biggl ( \frac{\DHKMhf(\bsk_i) \omega_i}{\lambda_{\bsk_i}} \biggr) _{i=1}^{n_1}}^\rho \right ].
\end{align*}
...
\begin{align*}
    ...
    \end{align*}
then $\norm[p\eta]{\bslambda} < \infty$ for all $\eta > 1$.  Since Algorithms \ref{DHKM:BallAlg} and \ref{DHKM:PilotConeAlg} are essentially optimal, the above inequalities provide upper and lower asymptotic bounds on  $\DHKMCOMP(\calB_R,\varepsilon,R')$ and $\DHKMCOMP(\calC_{\bslambda,n_1,A},\varepsilon,R)$.
\end{theorem}
\begin{proof}
...
From error bound \eqref{DHKM:APP_Err_Coef} it follows that the faster the $\DHKMhf(\bsk_i)$ decay, the faster $\DHKMAPP(f,n)$ converges to the solution.  Unfortunately, adaptive Algorithm \ref{DHKM:PilotConeAlg} does not adapt to the decay rate of the $\DHKMhf(\bsk_i)$ as $i \to \infty$. It simply bounds $\norm[\calF]{f}$ based on a pilot sample.  The algorithm presented in this section tracks the rate of decay of the $\DHKMhf(\bsk_i)$ and terminates sooner if the $\DHKMhf(\bsk_i)$ decay more quickly.  Similar algorithms for quasi-Monte Carlo integration are developed in \cite{HicJim16a}, \cite{JimHic16a}, and \cite{HicEtal17a}.
...
\sigma_j (f):=\norm[\rho]{\biggl(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \calK_j}} \qquad \text{for } j \in \N.
\end{equation}
...
\begin{equation} \label{DHKM:LambdaDef}
    \norm[\tau]{\bigl(\DHKMhf(\bsk) \bigr)_{\bsk \in \calK_j}} \le \sigma_j(f) \Lambda_j, \qquad \text{where } \Lambda_j : = \norm[\rho']{\bigl(\lambda_{\bsk} \bigr)_{\bsk \in \calK_j}}.
\end{equation}
...
\begin{align*}
    \norm[\calF]{f} & = \norm[\rho]{\Biggl(\,\norm[\rho]{\biggl(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \calK_j}}\,\Biggr)_{j \in \N}} = \bignorm[\rho]{\bigl(\sigma_j(f) \bigr)_{j \in \N}} \\
    & \le \bignorm[\rho]{\bigl(\sigma_1(f), ab \sigma_1(f), ab^2 \sigma_1(f), \ldots \bigr)} \\
    & = \left(1 + \frac{a^\rho b^\rho}{1 - b^\rho} \right)^{1/\rho}  \norm[\rho]{\biggl(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \calK_1}}.
\end{align*}
...
\nonumber &= \norm[\tau]{\left(\DHKMhf(\bsk_i) \right)_{i = n_j+1}^\infty}
= \norm[\tau]{ \left(\norm[\tau]{\bigl(\DHKMhf(\bsk) \bigr)_{\bsk \in \calK_l}} \right)_{l=j+1}^\infty}
\\
\nonumber
...
\begin{align*}
    \sigma_j(f) &= \left[ \sum_{i=n_0 2^{j-1} + 1}^{n_0 2^j} \biggabs{\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}}^\rho \right]^{1/\rho}
...
To illustrate the exponential case, suppose that $\DHKMhf(\bsk_i)/\lambda_{\bsk_i} = \Theta(\E^{-r_\Delta i})$.  For this exponential case one would normally define $\calC$ in terms of an arithmetic sequence, $(n_j)_{j\ge 0}$, e.g., $n_j = n_0 + j q$, where $q$ is a positive integer.  This implies that 
\begin{align*}
    \sigma_j(f) &= \left[ \sum_{i=n_0 + j q -q + 1}^{n_0 + j q} \biggabs{\frac{\DHKMhf(\bsk_i)}{\lambda_{\bsk_i}}}^\rho \right]^{1/\rho}
...
\begin{align*}
R &\ge \norm[\calF]{f} = \norm[\rho]{\left(\frac{\DHKMhf(\bsk)}{\lambda_{\bsk}} \right)_{\bsk \in \bbK}}
...
\begin{align}
\nonumber
\sigma_j(f_1) & = \norm[\rho]{\biggl(\frac{\DHKMhf_{1j}(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \calK_j}} 
...
\nonumber
\sigma_j(f_2) & = \norm[\rho]{\biggl(\frac{\DHKMhf_{2j}(\bsk)}{\lambda_{\bsk}} \biggr)_{\bsk \in \tcalK_j}} 
