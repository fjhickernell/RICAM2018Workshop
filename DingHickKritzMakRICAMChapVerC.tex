...
However, our goal is to choose a set $\calC$ of input functions and an algorithm $\ALG(f,\varepsilon)$ on $\calC$ that satisfies the error criterion \eqref{DHKM:err_crit}.  Such an algorithm will be adaptive, so that $\ALG(f,\varepsilon) = \APP(f,n)$.  The objectives of this chapter are to 
\begin{itemize}
...
\end{align*}
Comparing this inequality to the definition of $\DHKMchange{\calC_{\boldsymbol{\lambda},a,b}} $ in the previous section, it can be seen that  $\DHKMchange{\calC_{\boldsymbol{\lambda},A}} $ defined in \eqref{DHKM:TrackConeDef} is a subset of  $\calC$ defined in \eqref{DHKM:pilot_cone} if we choose 
