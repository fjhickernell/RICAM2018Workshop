...
\begin{document}
  ...
  %\subtitle{...}
  \abstract{We study adaptive approximation algorithms for multivariate linear 
  problems where the input sets are non-convex cones. While it is known that adaptive algorithms perform 
...
Consider the case of approximating functions defined over $[-1,1]^d$, using a Chebyshev polynomial basis.  The input function is denoted $f$, and the solution is $\SOL(f) = f$.  In this case,
	...
	\qquad \bsk = (k_1, \ldots, k_d),\\
    u_\bsk & := \prod_{\ell =1}^d \tu_{k_\ell} , 
    \qquad \tu_{k}(x) := \begin{cases} 1, & k =0 ,\\
	x, & k=1,\\
	2x \tu_{k-1}(x)-\tu_{k-2}(x), & k = 2, 3, \ldots.
	\end{cases}
	\end{align*}
Approximating $f$ well by a finite sum requires knowing which terms in the infinite series for $f$ are more important.  Let 
\begin{equation*}
...
\end{equation*}
where the $w_\ell$ are \emph{coordinate weights}.  The introduction of coordinate weights expresses the understanding that the $f$ may depend more strongly on some coordinates---those with larger coordinate weights---than others.  The definition of the norm of $f$ implies that an input function with modest sized norm must have series coefficients that decay quickly enough as the degree of the polynomial increases.  This decay of the series coefficients corresponds to the smoothness of the input functions.  Larger $r$ corresponds to greater smoothness.

The weights, $\lambda_{\bsk}$, can be ordered,
\begin{equation} \label{DHKM:lambda_order}
    \lambda_{\bsk_1} \ge \lambda_{\bsk_2} \ge \cdots >0,
\end{equation}
which implies an ordering of the wavenumbers, $\bsk$.  Given this ordering, it is natural to approximate the solution using the first $n$ series coefficients as follows:
\begin{equation*} %\label{DHKM:Ex_APP_def}
   \APP(f,n) := \sum_{i=1}^n \hf(\bsk_i) u_{\bsk_i} \qquad \forall n \in \N.
\end{equation*}

Here, we assume that it is possible to sample the series coefficients of the input function.  This is a less restrictive assumption than being able to sample any linear functional, but it is more restrictive than only being able to sample function values.  An important problem for future work is to extend the theory here to the case where the only function data available are function values.
...
If one has a fixed data budget, $n$, then $\APP(f,n)$ is the best answer.  However, our goal is an algorithm, $\ALG(f,\varepsilon)$ that satisfies the error criterion
...
To facilitate our derivations below, we establish the following lemma based on H\"older's inequality:
...
Also, we assume that all $\lambda_{\bsk}$ are  positive to avoid the trivial case where $\SOL(f)$ can be expressed exactly as a finite sum.
...
While approximation $\APP$ is a key piece of the puzzle, our ultimate goal is an algorithm, $\ALG : \calC \subset \calF \times [0,\infty)$, satisfying the absolute error criterion \eqref{DHKM:err_crit}. The non-adaptive Algorithm \ref{DHKM:BallAlg} satisfies this error criterion for $\calC  = \calB_R$.  
...
We define an algorithm to be \emph{essentially optimal} if there exists some fixed positive $\omega$, $\varepsilon_{\max}$, and $R_{\min}$ for which
...
The cone of functions for which our algorithms succeed, $\calC_d$, depends on the dimension too.  Also, $\SOL$, $\APP$, $\COST$, and $\COMP$ depend implicitly on dimension, and this dependence is sometimes indicated explicitly by the subscript $d$.
...
by $\log \varepsilon^{-1}$ in the previous tractability notions. A more detailed study of 
...
\end{align*}
Using the non-increasing ordering of the $\lambda_{\bsk_i}$, we employ a standard technique for bounding the $n+1^\text{st}$ largest $\lambda_{\bsk}$ in terms of the sum of the $1/p^\text{th}$ power of all the $\lambda_{\bsk}$.  For $0 < p < r$, 
\begin{align*}
(n+1)\lambda_{\bsk_{n+1}}^{1/p} &\le \sum_{i=1}^{n+1}\lambda_{\bsk_{i}}^{1/p} \le \sum_{\bsk \in \bbN_0^d}\lambda_{\bsk}^{1/p} = \prod_{\ell =1}^d \left[1 + w_\ell^{1/p}\sum_{k = 1}^\infty \frac {1}{k^{r/p}} \right ] \\
& =  \prod_{\ell =1}^d \left[1 + w_\ell^{1/p}\zeta(r/p)\right ] 
= \exp\left(\sum_{\ell = 1}^d \log\bigl(1 + w_\ell^{1/p}\zeta(r/p) \bigr) \right) \\
& \le \exp\left(\zeta(r/p) \sum_{\ell = 1}^{\infty} w_\ell^{1/p} \right) \qquad \text{since }\log(1+x) \le x \ \forall x\ge 0.
\end{align*}
...
\begin{align*}
    \MoveEqLeft{\textup{COMP}(\calA(\calB_{R}),\varepsilon,R)} \\
  &\le\min\left\{ n \in \N_0 : \frac{1}{n+1}\exp\left(\zeta(r/p) \sum_{\ell = 1}^\infty w_\ell^{1/p} \right)  \le \left( \frac{\varepsilon}{R} \right)^{1/p}  \right\} \\
  &= \left \lceil \left( \frac{R}{\varepsilon} \right)^{1/p}  \exp\left(\zeta(r/p) \sum_{\ell = 1}^{\infty} w_\ell^{1/p} \right) \right \rceil - 1.
\end{align*}


If the $w_\ell$ decay quickly enough so that $\sum_{\ell = 1}^{\infty} w_\ell^{1/p}$ is finite, we obtain strong polynomial tractability and a complexity that is $\calO \bigl((R/\varepsilon)^{1/p} \bigr)$.  On the other hand, if the coordinate weights are all unity,  $w_1 = w_2 = \cdots = 1$, then there are $2^d$ different $\lambda_{\bsk}$ with a value of $1$, and so $\COMP(\calA(\calB_{R}),\varepsilon,R) \ge 2^d$, and the problem is not tractable.
...
tells us something about what is not observed about the input function.  Let $n_1$ denote the number of pilot observations, based on the wavenumbers
\begin{equation} \label{DHKM:KOnedef}
    \calK_1 := \{\bsk_1, \ldots, \bsk_{n_1} \},
\end{equation}
where the $\bsk_i$ are given by the ordering of the $\lambda_{\bsk}$ in \eqref{DHKM:lambda_order}.  Let $A$ be some constant inflation factor greater than one.  The cone of functions whose norm can be bounded well in terms of a pilot sample, $\{\hf(\bsk_1), \ldots, \hf(\bsk_{n_1})\}$,
is given by
...
\[
    ...
    \le D_A\,  \varepsilon/R\, \right \},
\]
where $D_A$ is a positive constant that may depend on $A$, but does not depend on $d$, $\varepsilon$, and $R$. We would like to study under which conditions we obtain the various tractability notions defined in Section \ref{DHKM:secTractability}. 


Since $\lambda_{d,\bsk_i}>0$, and since we are interested in the asymptotic behavior 
...
n(\varepsilon,d)=\min \left \{n\ge 0: \bignorm[\rho']{\bigl(  \lambda_{d,\bsk_i}  \bigr)_{i = n+1}^{\infty}} \,
    \le \varepsilon/R \right \}\,\asymp\,\COMP(\calA(\calC_d),\varepsilon,R) ,
\]
for $\varepsilon \in (0,1)$ and $d\in\NN$, where the implied constant may depend on $a$.
To this end, we distinguish two cases, depending on whether $\rho'$ is infinite or not. This 
distinction is useful because it allows us to relate the computational complexity of the algorithms 
considered in this chapter to the computational complexity of linear problems on certain function spaces considered in the classical literature on information-based complexity, as for example \cite{NovWoz08a}. The case $\rho'=\infty$ corresponds to the
worst-case setting, where one studies the worst performance of an algorithm over the unit ball of 
...
\PeterNote{I added a sentence for $\rho'=\infty$ regarding similarities to the 
case of balls in the worst-case setting. For $\rho'<\infty$ it does not make sense to speak of 
balls, because this case is related to the average case setting, where one considers the average 
...
\[
\FredNote{n(\varepsilon/R,d)=\min \left \{n\ge 0\colon \lambda_{d,\bsk_{n+1}} \,
...
\FredNote{In this case, letting $\widetilde{\varepsilon}:=(\varepsilon/R)^{\rho'/2}$ and 
...
\YuhanNote{\FredNote{
To be more concrete, we consider the situation where the $\lambda_{\bsk}$ are specified in terms of \emph{coordinate weights}, $w_1, \ldots, w_d$, and \emph{smoothness weights}, $s_1, s_2, \ldots$:
\begin{equation}
    \label{DHKM:prodwts}
\lambda_{d,\bsk}  := \prod_{\substack{\ell =1\\ k_\ell > 0}}^d w_\ell s_{k_\ell}, \qquad \bsk \in \NN_0^d, \ d \in \NN.
\end{equation}
This is a generalization of what was considered in Section \ref{DHKM:secexamp}, where $s_{k} = k^{-r}$.  This form of the $\lambda_{d,\bsk}$ is considered in greater detail in Section \ref{DHKM:smoothimportance}.
The same argument as in Section \ref{DHKM:revisexamp} implies that
...
For the $\lambda_{d,\bsk}$ of the form \eqref{DHKM:prodwts} we have strong polynomial tractability if there exists $\eta>0$ such that
 \begin{equation}
...
\end{equation}
\end{corollary}
\textbf{I don't know if we can get if and only if.  Yuhan and Peter please check.}}}

\PeterNote{For $d=1$, we need $\sum_{i=1}^\infty \lambda_{1,k_i}^\eta$ to be finite. But is not 
$\sum_{i=1}^\infty \lambda_{1,k_i}^\eta=w_1\sum_{k=1}^\infty s_k^\eta$ ? If I am right, this shows that 
$\sum_{k=1}^\infty s_k^\eta<\infty$ is necessary. I am not sure if the second condition is necessary. 
Need some time to think.}

\FredNote{Yes, I think that you are correct.  If we assume that $s_1>1$, then we may be able to show that the condition on the coordinate weights is necessary.}
...
\end{theorem}
\begin{proof}
Let $j^\dagger(\varepsilon)$ be defined as in \eqref{DHKM:TractConejdagger}, with the $\varepsilon$ dependence made explicit.  \FredNote{Choose $\varepsilon_{\max}$ and $R_{\min}$ in \eqref{DHKM:EssentialOpt} such that $j^\dagger(\varepsilon) \ge 2$.}  This definition implies that
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Inferring Coordinate and Smoothness Importance} \label{DHKM:smoothimportance}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In Sections \ref{DHKM:pilot_sec} and \ref{DHKM:tracking_sec} the $\lambda_{\bsk}$, which appear in the definition of the cone of inputs, $\calC$ are taken as given.  These may take the form suggested in \eqref{DHKM:prodwts}, which define the $\bsk$ in terms of coordinate weights and smoothness weights.  However, practically speaking it may be difficulty to know a priori the values of these weights.
This section explores a situation where the initial data collected for the input function data can be used to learn the $\lambda_{\bsk}$, inferring which input variables in $f$ may be more important and the smoothness of the function.  

The motivation for this section is situations where the relative importance of the $d$ input variables of the function is not known from physical considerations. We also envision the situation where the cost of function data is large, e.g., the result of an expensive computer simulation.  Thus, we are not concerned with the cost of algorithm beyond the information cost, which we hope to limit to $\calO(d)$.
...
\begin{itemize}
    ...
    \item Smoothness weights quantify the importance of the $\tu_k$.  E.g., if the $\tu_k$ is a polynomial  of degree $k$, then the faster the $s_k$ decay, the smoother $f$ is.  
...
\begin{itemize}
\item \emph{Effect sparsity} assumes that only a small number of inputs in $f$ are important. In \eqref{eq:posd}, this sparsity is accounted for by allowing only a small number of product weights $\bsw$ to be large.  This principle arose in the sufficient condition for strong tractability in Corollary \ref{DHKM:sptexample_cor}.
\item \emph{Effect heredity} assumes that lower-order effects are more important than higher-order effects. E.g., $\lambda_{(1,0, 0,\ldots, 0)}$ should be larger than $\lambda_{(1, 1, 0, \ldots, 0)}$. In \eqref{eq:posd}, this heredity can be enforced by letting order weights $\Gamma_{m}$ be decreasing in $m$. 
\item \emph{Effect hierarchy} assumes that an effect is active \emph{only} when all its component effects are active. For example, $\lambda_{(1, 1, 0, \ldots, 0)} > 0$ only when $\lambda_{(1, 0, 0, \ldots, 0)}$ and $\lambda_{(0, 1, 0, \ldots, 0)}$ are both nonzero. This hierarchy is implicitly enforced within the product structure of the weights in \eqref{eq:posd}.
\item \emph{Effect smoothness} assumes that lower-degree effects are more important than higher-degree effects. For example, when the $(\tu_k)_{k \in \mathbb{N}_0}$ are polynomials, this means that linear effects are more important than quadratic effects, which are in turn more significant than cubic effects, and so on. Effect smoothness can be imposed by setting $\bss$ to be a decreasing sequence. 
\end{itemize}

The $\lambda_{\bsk}$ defined in \eqref{eq:posd} are called product, order and smoothness dependent (POSD) weights.  From a quasi-Monte Carlo (QMC) perspective, the POSD weights in \eqref{eq:posd} generalize upon the product-and-order dependent (POD) weights in \cite{KuoEtal12a}, which were introduced for analyzing QMC methods in partial differential equations with random coefficients. The latter POD weights can be recovered by ignoring the smoothness weights. Our POSD weights differ from the smoothness-driven product-and-order dependent (SPOD) weights in \cite{Dea2014}, which were recently used to analyze higher-order QMC methods for stochastic partial differential equations. These SPOD weights take the form:
\begin{equation}
\gamma_{\fraku} = \sum_{\bsk \in \{1:\alpha\}^{|\fraku|}} \|\bsk\|_1 ! \prod_{l \in \fraku} \left( 2^{\delta(k_\ell,\alpha)} w_\ell^{k_\ell} \right), \quad \|\bsk\|_1 = \sum_{l=1}^d k_\ell, \quad \fraku \in \{0, 1\}^d.
\label{eq:spod}
\end{equation}
Intuitively, the SPOD weights in \eqref{eq:spod} quantify the importance of each \textit{subspace} (indexed by $\fraku$), under a common smoothness structure between subspaces. In contrast, the proposed POSD weights in \eqref{eq:posd} instead quantify the importance of each \textit{Fourier coefficient} in $f$ (indexed by $\bsk$), under a common smoothness structure between coefficients.
...
Algorithm \ref{DHKM:InfPilotConeAlg} combines the construction of data-inferred POSD weights, $\bar{\bslambda}$ with Algorithm \ref{DHKM:PilotConeAlg}. This algorithm succeeds for input functions in the cone 
...
\begin{algorithm}
	...
	\begin{algorithmic}
	\PARAM candidate sets $\calW$ and $\calC$; maximum smoothness degree, $k_{\max}$; order weights, $\boldsymbol{\Gamma}$; an inflation factor, $A > 1$; $\APP$ satisfying \eqref{DHKM:APP_errorBd}; 
		\INPUT a black-box function, $f$; an absolute error tolerance,
		$\varepsilon>0$
...
\State Using these weights, perform Algorithm \ref{DHKM:PilotConeAlg} to obtain $\ALG(f,\varepsilon)$
\RETURN $\ALG(f,\varepsilon)$
\end{algorithmic}
\end{algorithm}


\FredNote{Fred is here}

\subsection{Numerical Examples}

We now investigate the numerical performance of this adaptive algorithm using data-inferred POSD weights. For simplicity, only the case of $\rho = \infty$ and $\rho' = \tau = 1$ is considered in the following examples. Here, the basis functions $(u_\bsk)_{\bsk \in \mathbb{N}_0^d$ are Chebyshev polynomials, and the solution operator is $\SOL (f) = f$ (i.e., function approximation). We note that $\|f - \APP(f)\|_\infty$ is bounded above by $\|f - \APP(f)\|_\calG$, so the worst-case approximation error $\|f - \APP(f)\|_\infty$ should satisfy the desired error tolerance $\varepsilon$ by \eqref{DHKM:err_crit}.
...
\begin{equation}
\hat{f}(\bsk) = Z_{\bsk} \left( {\Gamma}_{\|\bsk\|_0}^{\rm tr} \prod_{\substack{l=1\\ k_\ell>0}}^d {w_\ell^{\rm tr}} {s}_{k_\ell}^{\rm tr} \right), \quad Z_{\bsk} \overset{i.i.d.}{\sim} \text{Unif}[-1, 1], \quad \bsk \in \mathbb{N}_0^d.
\label{eq:foursim}
\end{equation}
Here, $(w_\ell^{\rm tr})_{l=1}^d = (1/l^2)_{l=1}^d$, $({\Gamma_k^{\rm tr}})_{k=1}^\infty \equiv 1$ and $(s_j^{\rm tr})_{j=1}^{k_{\rm max}} = (1/j^4)_{j=1}^4$ are the true product, order and smoothness weights of $f$, and $Z_{\bsk}$ randomly sets the magnitude and sign of each coefficient. Product weights are also randomized in terms of variable order, to ensure that the order of input variables does not necessarily reflect the order of variable importance. We also set $\boldsymbol{\Gamma}$ in Algorithm \ref{DHKM:InfPilotConeAlg} as the true order weights $\boldsymbol{\Gamma}^{\rm tr}$, and use an inflation factor of $A = 1.1$.

%\PeterNote{Below, we write $\epsilon instead of $\varepsilon$. Simon, is that on purpose?}

Figures \ref{fig:four} (a) and (b) plot the total required sample size from Algorithm \ref{DHKM:InfPilotConeAlg}, as a function of the error ratio $\|f - \APP(f)\|_\infty/\varepsilon$, for $f$ in $d=4$ and $d=7$ dimensions, respectively. Each data point corresponds to a different error tolerance $\varepsilon$. Here, a ratio $\|f - \APP(f)\|_\infty / \varepsilon$ close to 1 (but not exceeding 1) is desired, since this shows our adaptive procedure can achieve the desired tolerance $\varepsilon$ by inferring and incorporating POSD structure. For $d=4$ (Figure \ref{fig:four} (a)), $\|f - \APP(f)\|_\infty/\varepsilon$ fluctuates around 0.4 for all choices of $\varepsilon$; for $d=7$ (Figure \ref{fig:four} (b)), this ratio begins at $\approx 0.1$ for $\varepsilon = 0.1$, then decrease to $\approx 0.014$ for $\varepsilon = 0.001$. This shows that our adaptive approximation algorithm works reasonably well, and appears slightly more effective in lower dimensions ($d=4$) than in higher dimensions ($d=7$). A likely reason for this is that the underlying POSD structure can be more easily learned from a small pilot sample in lower dimensions than in higher dimensions; using this inferred structure then allows for a tighter data-based error bound.
...
\end{subfigure}
\caption{Total required sample size as a function of error ratio $\|f - \hat{f}\|_{\infty}/\varepsilon$, with points colored by the absolute error tolerance level $\varepsilon$.}
\label{fig:four}
\end{figure}
