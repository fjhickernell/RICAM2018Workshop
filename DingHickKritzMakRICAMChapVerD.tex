...
\end{equation*}
for some constants $C_{\lo}$ and $C_{\up}$.  \DHKMchange{By reasonable we mean that the values of $\sigma_j(f)$ would not drop precipitously and then rise again as $j$ increases.} Choosing $a \ge C_{\up}/C_{\lo} $  and $b \ge  2^{-(r_\Delta-1/\rho)}$ causes the cone $\DHKMchange{\calC_{\boldsymbol{\lambda},\bsn,a,b}}$ to include such functions.  Note that only the ratio of $C_{\up}$ to $C_{\lo}$ need be assumed to determine $a$, and choosing $b$ larger than necessary does not affect the order of the decay of the error bound. 
...
\end{align*}
From \eqref{DHKM:TractConejdagger} in Theorem \ref{DHKM:TrackConeCompCost} it follows that the cost of Algorithm \ref{DHKM:TrackConeAlg} has an upper bound of $\COST(\ALG,\calC_{\bslambda,a,b},\varepsilon,R) \le n_j = n_0 2^j$ \SimonNote{Should this be $\COST(\ALG,\calC_{\bslambda,\bsn,a,b},\varepsilon,R)$?} for any $j$ satisfying 
\begin{align*}
