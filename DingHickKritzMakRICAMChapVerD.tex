...
\FredNote{As far as simplifications go, 

1) If we like the $\bslambda^{(n)}$ notation and even want to write $\bslambda^{(n)}_d$ to emphasize the $d$ dependence, then we can simplify some of the equations below.}

\PeterNote{I am not sure. I have the impression that with the additional restriction 
that many sums do not start with index 1, but with indices depending on $d$ and other parameters, it might make notation overly complicated... .
I mostly left it as it is. But if you prefer, we can still change it!}

\FredNote{I commented out where I thought we could introduce the notation.  Let me know what you think.}

\PeterNote{I agree. Let us use your notation, it makes it more consistent.}
...
%     \sup_{d\in\NN} \sum_{i=i_0}^\infty \lambda_{d,\bsk_i}^p < \infty.
%  \end{equation}
 \begin{equation}\label{DHKM:eq:condspt}
...
%  \[
%     \sup_{d\in\NN} d^{-p_1}\, \sum_{i=\lceil K d^{p_2} \rceil}^\infty \lambda_{d,\bsk_i}^{p_3} < \infty.
%  \]
 \[
...
%     \sup_{d\in\NN} \sum_{i=i_0}^\infty \lambda_{d,\bsk_i}^{\rho'\,p} < \infty.
%  \end{equation}
 \begin{equation}\label{DHKM:eq:condspt1}
...
%  \[
%     \sup_{d\in\NN} d^{-p_1}\, \sum_{i=\lceil K d^{p_2} \rceil}^\infty \lambda_{d,\bsk_i}^{\rho'\,p_3} < \infty.
%  \]
  \[
    \sup_{d\in\NN} d^{-p_1}\, \bignorm[\rho' p_3]{\bslambda_d^{(\lceil K d^{p_2} \rceil)}} < \infty.
 \]
 \item[3.] Let $t_{d,i}:=\sum_{\ell=i}^\infty \lambda_{d,\bsk_{\ell}}$. \FredNote{Here we have a problem since $i$ is both a summation index and limit.} \PeterNote{I think it should be ok now (?).}
 We have weak tractability if and only if 
 \[
...
% Below, we write $\bslambda_d$ for the 
% sequence of the $\lambda_{d,\bsk}$
% Considering Condition \eqref{DHKM:eq:condspt} in Theorem \ref{DHKM:thmtract1}, note that
% \[
% \sup_{d\in\NN} \sum_{i=i_0}^\infty \lambda_{d,\bsk_i}^p = \sup_{d\in\NN} \norm[p]{\bslambda_d^{(i_0)}}^p.
% \]
% Since we assumed that the $\lambda_{d,\bsk}$ are ordered, 
