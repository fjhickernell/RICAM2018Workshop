...
\DHKMchange{
We now interpret the information cost upper bounds of Theorems \ref{DHKM:thm_cost_non_adapt} and \ref{DHKM:PilotCostThm} for the example introduced in Section \ref{DHKM:secexamp}, where $\rho = \tau = 2$ and $\rho' = \infty$.  In this case, $\bignorm[\rho']{\bigl(  \lambda_{\bsk_i}  \bigr)_{i = n+1}^{\infty}} = \lambda_{\bsk_{n+1}}$.  To facilitate this interpretation we establish a couple of commonly used lemmas.
...
then we have
\[ 
\lambda_{\bsk_{n+1}} \le \frac{\norm[p]{\bslambda}}{n^{1/p}} \qquad \forall p >0,
\]
provided that $\norm[p]{\bslambda}$ is finite.
\end{lemma}
\begin{proof}
...
\begin{equation*}
    ...
    + \lambda_{\bsk_{n}}^p  \le \sum_{\bsk \in \bbK}^\infty \lambda_{\bsk}^p = \norm[p]{\bslambda}^p \qquad \forall p > 0.
\end{equation*}
Dividing both sides by $n$ and taking the $p^{\text{th}}$ root completes the proof.
\end{proof}
...
\begin{equation*}
    ...
    \end{multline*}
Note that these costs are $\calO(R^p \varepsilon^{-p})$ for all $p > 1/r$. A faster decay rate of the $\lambda_{\bsk}$ due to the faster decay rate of the smoothness coefficients implies smaller information cost.
}

\FredNote{Fred got to here}

\FredNote{I think that we need a lower bound on $\lambda_{\bsk_{n+1}}$ to obatain lower bounds on the computational complexity. }
