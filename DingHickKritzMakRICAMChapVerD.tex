...
Adaptive algorithms infer the necessary computational effort based on the function data sampled.  Adaptive algorithms may perform better than non-adaptive algorithms if the set of input functions is \emph{non-convex} \DHKMchange{or \emph{non-symmetric}}. \DHKMchange{Here we focus on the former situation and} construct adaptive algorithms for general multivariate linear problems where the input functions lie in non-convex cones. \DHKMchange{By a cone, we mean a set of functions satisfying that for each element also all its multiples lie in the set. We will study different variants of cone here, the precise definitions will be given below.}

Our algorithms use a finite number of series coefficients of the input function to construct an approximate solution that satisfies an absolute error tolerance.  We show our algorithms to be essentially optimal.  We derive conditions under which the problem is tractable, i.e., the information cost of constructing the approximate solution does not increase exponentially with the dimension of the input function domain.  In the remainder of this section we outline the general idea of the paper. 
...
We call an algorithm \emph{non-adaptive} if it is such that $n(f,\varepsilon)=n(\varepsilon)$, independently of the particular choice of $f\in\calC$. 

The information cost of $\ALG(f,\varepsilon)$ is denoted $\COST(\ALG,f,\varepsilon)$ and defined as the number of function data---in our situation, series coefficients---required by $\ALG(f,\varepsilon)$.  For adaptive algorithms this cost varies with the input function $f$.  
...
\[\calC'_{\boldsymbol{\lambda}, \boldsymbol{\omega}, A'}=\{ f \in \cf : \norm[\calF]{f} \le A'\norm[\calF']{f}\},\]
where
$\norm[\calF']{f} =\norm[\rho]{\left( \frac{\hf(\bsk)\omega_{\bsk}}{\lambda_{\bsk}} \right)_{\bsk \in \mathbb{K}}}$ and $\omega_{\bsk}$ decreases. 
\begin{proposition}
...
The same argument as \DHKMchange{stated above} implies that the sum of the $\lambda_{d,\bsk_i}^\eta$ is bounded above as
\[
