...
\end{multline}
\end{subequations}
This error bound decays as $\bignorm[\rho]{\bigl( f(\bsk_i)/\lambda_{\bsk_i} \bigr)_{i=1}^n}$ increases and as the tail norm of the $\lambda_{\bsk_i}$ decreases.  This data-driven error bound underlies  Algorithm \ref{DHKM:PilotConeAlg}, which is successful for $\DHKMchange{\calC_{\boldsymbol{\lambda},n_1,A}}$ defined in \eqref{DHKM:pilot_cone}:
...
\DHKMchange{Let us consider an alternative cone, i.e.,
\begin{equation} \label{DHKM:altconedef}
    \calC'_{\boldsymbol{\lambda}, \boldsymbol{\omega}, A'}=\{ f \in \calF : \norm[\calF]{f} \le A'\norm[\calF']{f}\},
\end{equation}
where
$\norm[\calF']{f} =\norm[\rho]{\left( \frac{\hf(\bsk)\omega_{\bsk}}{\lambda_{\bsk}} \right)_{\bsk \in \mathbb{K}}}$ and $\omega_{\bsk}$ decreases. The norm $\norm[\calF']{\cdot}$ is weaker than the norm $\norm[\calF]{\cdot}$, so this cone is defined as the subset of $\calF$ for which the weaker norm of the function  provides an upper bound the stronger norm of the function.  The following proposition describes the relationship between the cones defined in \eqref{DHKM:pilot_cone} and \eqref{DHKM:altconedef}.
\begin{proposition}
If there exist $A,A',A'', n_1,$ and $\boldsymbol{\omega}$ such that
\[\omega_{n_1} A'' \ge A \ge \omega_1A'\left[\frac{1 + (\omega_{n_1+1}/\omega_1)^\rho}{1 - (\omega_{n_1+1}A')^\rho}\right]^{1/\rho} \text{ and} \qquad  \omega_{n_1+1}A' < 1, \]
then 
\[\calC'_{\boldsymbol{\lambda}, \boldsymbol{\omega}, A''}
\subseteq 
\calC_{\boldsymbol{\lambda}, A}
\subseteq
\calC'_{\boldsymbol{\lambda}, \boldsymbol{\omega}, A'}.\]
\end{proposition}

\begin{proof}
Let us prove $\calC'_{\boldsymbol{\lambda}, \boldsymbol{\omega}, A''}
\subseteq 
\calC_{\boldsymbol{\lambda}, A}$ at first.
Suppose $f \in \calC_{\boldsymbol{\lambda}, A}$, then by definition we have
\begin{align*}
    \norm[\calF]{f} &\le A \norm[\rho]{\left( \frac{\hf(\bsk)}{\lambda_{\bsk}} \right)_{\bsk = 1} ^{ n_1}}
     \le A \omega_{n_1}^{-1} \norm[\rho]{\left( \frac{\hf(\bsk)\omega_{\bsk}}{\lambda_{\bsk}} \right)_{\bsk =1} ^{n_1}}
     \le A \omega_{n_1}^{-1} \norm[\calF']{f}.
\end{align*}
Thus, if $A'' \ge A\omega_{n_1}^{-1}$, i.e. $\omega_{n_1} A'' \ge A $,
we obtain $\calC'_{\boldsymbol{\lambda}, \boldsymbol{\omega}, A''}$.

For the proof of the remaining part, 
...
\YuhanNote{Create a new subsection and move the content related with the example here}
\DHKMchange{
Before we revisit the example in Section \ref{DHKM:secexamp}, we introduce several lemmas.

\begin{lemma}\label{DHKM:lambdineq}
Consider the ordered weights,
$\lambda_{\bsk_{1}} \ge \lambda_{\bsk_{2}} \ge \cdots > 0,$
then we have for $p>0$,
\[ \sum_{i=1}^\infty \lambda_{d,\bsk_{i}}^p \le \frac{1}{n^{1/p}}
\left(\sum_{\bsk \in \NN_0^d}^\infty \lambda_{\bsk}^p\right)^{1/p}\].
\end{lemma}
\PeterNote{It seems to me that on the left-hand side of the 
above inequality we need $\lambda_{\bsk_{n+1}}^p$, right?}
\begin{proof}
As $\lambda_{\bsk_{1}} \ge \lambda_{\bsk_{2}} \ge \cdots > 0$, we obtain for $p>0$
\begin{align*}
    n \lambda_{\bsk_{n+1}}^p & \le 
    \lambda_{\bsk_{1}}^p + \lambda_{\bsk_{2}}^p + \cdots 
    + \lambda_{\bsk_{n}}^p  \le \sum_{\bsk \in \NN_0^d}^\infty \lambda_{\bsk}^p \\
    \Longleftrightarrow 
    \lambda_{\bsk_{n+1}}&  \le \frac{1}{n^{1/p}}
\left(\sum_{\bsk \in \NN_0^d}^\infty \lambda_{\bsk}^p \right)^{1/p}.
\end{align*}
\end{proof}
...
\end{equation}
To stress the dependence on $d$, we write $\lambda_{d,\bsk_i}$ instead of $\lambda_{\bsk_i}$
  This form of the $\lambda_{d,\bsk}$ is considered in greater detail in Section \ref{DHKM:smoothimportance_sec}.

\begin{lemma}\label{DHKM:prodwtsupperbound}
Consider $\lambda_{d,\bsk}$ defined in \eqref{DHKM:prodwts},
we have for $p>0$,
\[\sum_{\bsk \in \NN_0^d}^\infty \lambda_{d,\bsk}^p \le \exp\left(\sum_{k=1}^\infty s_k^p \sum_{\ell = 1}^{d} w_\ell^{p} \right)\le \exp\left(\sum_{k =1 }^{\infty} s_k^p \sum_{\ell = 1}^{\infty} w_\ell^{p} \right),\]
if $\sum_{\ell = 1}^{\infty} w_\ell^{p}$ is finite.
\end{lemma}
\begin{proof}
\begin{align*}
   \sum_{\bsk \in \NN_0^d}^\infty \lambda_{d,\bsk}^p  & =
   \prod_{\ell =1}^d\left(1+ w_\ell^p \sum_{k=1}^{\infty}s_{k}^p\right) \qquad \text{ by \eqref{DHKM:prodwts} }\\
    & = \exp\left(\sum_{\ell = 1}^d \log\bigl(1 + w_\ell^{p}\sum_{k=1}^{\infty}s_{k}^p \bigr) \right)\\
 &  \le \exp\left(\sum_{\ell = 1}^d  w_\ell^{p}\sum_{k=1}^{\infty}s_{k}^p \right)
\qquad \text{since }\log(1+x) \le x \ \forall x\ge 0,\\
&\le \exp\left(\sum_{\ell = 1}^\infty w_\ell^{p}\sum_{k=1}^{\infty}s_{k}^p \right)
\qquad \text{ since  } \sum_{\ell = 1}^{\infty} w_\ell^{p} \text{ converges}.\\
\end{align*}
\end{proof}

The example in Section \ref{DHKM:secexamp} chooses $\rho = \tau = 2$, $\rho' = \infty$. Thus, we obtain by Theorem \ref{DHKM:thm_cost_non_adapt}:
\begin{align*}
	\COMP(\calA(\calB_{R}),\varepsilon,R) &= \COST(\ALG,\calB_{R},\varepsilon,R) 
	=\min\{ n \in \N_0 : \lambda_{\bsk_{n+1}} \le \varepsilon/R \}.
\end{align*}
Using the non-increasing ordering of the $\lambda_{\bsk_i}$, we employ a standard technique for bounding the $n+1^\text{st}$ largest $\lambda_{\bsk}$ in terms of the sum of the $p^\text{th}$ power of all the $\lambda_{\bsk}$.  For $0  < 1/r < p$, by Lemma \ref{DHKM:lambdineq} and Lemma \ref{DHKM:prodwtsupperbound} with $s_k = k^{-r}$, 
\begin{align*}
n\lambda_{\bsk_{n+1}}^{p} \le \sum_{\bsk \in \bbN_0^d}\lambda_{\bsk}^{p}  \le 
\exp\left(\sum_{\ell = 1}^d  w_\ell^{p}\sum_{k=1}^{\infty}\frac{1}{k^{p r} }\right)
 \le \exp\left(\zeta(pr) \sum_{\ell = 1}^{\infty} w_\ell^{p} \right).
\end{align*}
...
\begin{align*}
  ...
  &\le\min\left\{ n \in \N_0 : \frac{1}{n}\exp\left(\zeta(p r) \sum_{\ell = 1}^\infty w_\ell^{p} \right)  \le \left( \frac{\varepsilon}{R} \right)^{p}  \right\} \\
  &= \left \lceil \left( \frac{R}{\varepsilon} \right)^{p}  \exp\left(\zeta(p r) \sum_{\ell = 1}^{\infty} w_\ell^{\eta} \right) \right \rceil .
\end{align*}
...
Consider the example in Section \ref{DHKM:secexamp}, we have
\begin{align*}
    \MoveEqLeft{\COMP(\calA(\calB_{R}),\varepsilon,R)} 
...
Using the same notation as above, the following statements hold for the case $\rho'=\infty$.
 ...
 \item[2.] 
  We have polynomial tractability if and only if there exist $\eta_1, p_2 \ge 0$ and $\eta_3, K>0$ such that
 \[
...
\DHKMchange{
To be more concrete, we consider the situation 
\[
\lambda_{d,\bsk}  := \prod_{\substack{\ell =1\\ k_\ell > 0}}^d w_\ell s_{k_\ell}, \qquad \bsk \in \NN_0^d, \ d \in \NN,
\] one more time.
...
\begin{align*}
    \sup_{d \in \N} \, \sum_{i=i_0}^{\infty}\lambda_{d, \bsk_{i}}^{\eta} & \ge w_1^\eta \sum_{\DHKMchange{i=i_0}}^{\infty}s_{\DHKMchange{k_i}}^{\eta} \qquad \parbox{8cm}{considering only $\bsk_i$ of the form $(k, 0, 0, \ldots, 0)$, \\
    and ordering $\bss$ so that $\DHKMchange{s_{k_1} \ge s_{k_2} \ge \cdots}$,} \\
...
\end{equation*}
\DHKMchange{In particular, considering example in Section \ref{DHKM:secexamp}, where $s_{k} = k^{-r}$, we have strong polynomial tractability if and only if there exists $\eta> 0 $ such that
\begin{equation*}
%\label{DHKM:sptexample}
   r \eta > 1 \text{ and } 
    \sum_{\ell = 1}^{\infty} w_\ell^{\eta} < \infty.
...
\begin{equation} \label{DHKM:TrackConeDef}
  \DHKMchange{\calC_{\boldsymbol{\lambda},a,b}} : =\left\{f\in\calF \colon \sigma_{j+r} (f)\le ab^r \sigma_j (f)\ \forall j,r\in\NN\right\}.
\end{equation}
Here, $a$ and $b$ are positive reals with $b< 1 < a$. The constant $a$ is an inflation factor, and the constant $b$ defines the general rate of decay of the $\sigma_j(f)$ for $f \in  \DHKMchange{\calC_{\boldsymbol{\lambda},a,b}} $. 
...
Because $ab^r$ may be greater than one, we do not require the series coefficients of the solution, $\SOL(f)$, to decay monotonically. However, we expect their partial sums to decay steadily.  The series coefficients for wavenumbers $\bsk \in \calK_0$ do not affect the definition of $\DHKMchange{\calC_{\boldsymbol{\lambda},a,b}} $ and may behave erratically.
...
\YuhanNote{Create a new version of Theorem 6, as $ \frac{1 - b^{j\rho}}{1 - b^\rho} \ge 1 \forall j$. And based on that, derive the explicit bound of the example. }
\DHKMchange{
...
Let's revisit the example in Section \ref{DHKM:secexamp}. For this example,
...
For example, we have $\rho = \tau = 2$, $\rho' = \infty$. Therefore
