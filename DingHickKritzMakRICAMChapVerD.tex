...
\begin{equation} \label{DHKM:TrackConeDef}
  \DHKMchange{\calC_{\boldsymbol{\lambda},\bsn,a,b}} : =\left\{f\in\calF \colon \sigma_{j+s} (f)\le ab^s \sigma_j (f)\ \forall j,s\in\NN\right\}.
\end{equation}
...
Because $ab^s$ may be greater than one, we do not require the series coefficients of the solution, $\SOL(f)$, to decay monotonically. However, we expect their partial sums to decay steadily.  The series coefficients for wavenumbers $\bsk \in \calK_0$ do not affect the definition of $\DHKMchange{\calC_{\boldsymbol{\lambda},\bsn,a,b}} $ and may behave erratically.
...
\nonumber 
&
= \norm[\tau]{ \bigl(\sigma_{j+s}(f) \Lambda_{j+s} \bigr)_{s=1}^\infty}
\\
& \le a \sigma_j(f) \norm[\tau]{ \bigl(b^s\Lambda_{j+s} \bigr)_{s=1}^\infty} =:\ERRNj
...
\YuhanNote{Create a new version of Theorem \ref{DHKM:TractConeCompCost}, as $ \frac{1 - b^{j\rho}}{1 - b^\rho} \ge 1 \forall j$. And based on that, derive the explicit bound of the example. }
\DHKMchange{
...
Moreover, $\COST(\ALG,\DHKMchange{\calC_{\boldsymbol{\lambda},\bsn,a,b}},\varepsilon,R) \le n_{j^\dagger}$, where $j^\dagger$ is defined as follows:
\begin{equation} \label{DHKM:TractConejdagger}
j^\dagger = \min \left \{j \in \bbN :   \norm[\tau]{ \bigl(b^{s}\Lambda_{j+s} \bigr)_{s=1}^\infty}
...
& \ge \sigma_{j^\dagger}(f)\,\frac{Ra}{\varepsilon} \norm[\tau]{ \bigl(b^s\Lambda_{j^\dagger+s} \bigr)_{r=1}^\infty}
\qquad \text{by the definition of } j^\dagger \text{ in \eqref{DHKM:TractConejdagger}} \\
& = \frac{R}{\varepsilon} \ERRNjd\, .
\end{align*}
From this last inequality, it follows that $j^\dagger \ge j^*$.
...
\begin{align*}
\norm[\rho]{ \bigl(b^{s}\Lambda_{j+s} \bigr)_{s=1}^\infty}^\rho
 & =  \sum_{s=1}^\infty
( b^{j+s}\Lambda_{j+s})^\rho \\
& \le \sum_{s=1}^\infty  
 \frac{b^{s\rho} \norm[p]{\bslambda}^\rho }{ n_0^{\rho/p} 2^{(j+s-1)\rho/p}} 
=  
 \frac{b^{\rho} \norm[p]{\bslambda}^\rho }{ n_0^{\rho/p} 2^{j\rho/p} (1 - b^{\rho}/2^{\rho/p})}, \\
\norm[\rho]{ \bigl(b^{s}\Lambda_{j+s} \bigr)_{s=1}^\infty}
 & \le  
 \frac{b \norm[p]{\bslambda} }{ n_0^{1/p} 2^{j/p} (1 - b^{\rho}/2^{\rho/p})^{1/\rho}}.
\end{align*}
From \eqref{DHKM:TractConejdagger} in Theorem \ref{DHKM:TractConeCompCost} it follows that the cost of Algorithm \ref{DHKM:TrackConeAlg} has an upper bound of $\COST(\ALG,\calC_{\bslambda,a,b},\varepsilon,R) \le n_j = n_0 2^j$ for any $j$ satisfying 
\begin{align*}
...
The following theorem establishes a lower bound on the complexity of our problem for input functions in $\DHKMchange{\calC_{\boldsymbol{\lambda},\bsn,a,b}}$. The theorem after that shows that the cost of our algorithm as given in Theorem \ref{DHKM:TractConeCompCost} is essentially no worse than this lower bound.
